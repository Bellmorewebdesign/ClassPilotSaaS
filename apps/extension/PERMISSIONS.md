# Why Coursen AI asks for each permission

Coursen AI requests the minimum set that makes a Classroom sync possible.
...
|---|---|
| `storage` | Stores your Coursen AI API URL and API token in `chrome.storage.local`, and the last sync summary shown in the popup. This is profile-local and is **not** synced to your Google account. |
...
| `https://classroom.google.com/*` (host) | Lets the content script read the Classroom pages you are already viewing. This is the only site Coursen AI reads. |
| `http://*/*`, `https://*/*` (**optional**) | Requested *only* when you save a custom API URL, so the extension can POST your synced data to your own Coursen AI API. Not granted at install time. Declining still works in the normal case, because the API allows `chrome-extension://` origins via CORS. |
...
- **`identity` / OAuth scopes.** Coursen AI does not authenticate to Google at
  all. It relies entirely on the session already in your browser.
- **`cookies`.** Coursen AI never reads your Google cookies. The helper tab
  carries your existing session the same way a normal tab would.
- **`webRequest`.** No network interception.

## What leaves your browser

Only the normalized Classroom data you see in the Coursen AI dashboard —
